...
- 页面包含：
  ...
  - **右下角摄像头预览框**（240×180 像素，带白边和圆角，实时显示用户手部画面，用于反馈手势识别到的手势）
  - 摄像头预览框开关，可以关闭/开启摄像头预览
  - 地球旋转灵敏度，用于控制挥手旋转地球灵敏度

#### 🌍 地球与场景
- 使用 Three.js创建带纹理的**3D卫星地球模型**，参考NASA的地球模型：https://solarsystem.nasa.gov/gltf_embed/2393/
- 相机为 `PerspectiveCamera`，初始位置 `(0, 0, 5)`，目标 `(0, 0, 0)`
- 背景添加宇宙环境，不需要添加额外的宇宙光线，开启抗锯齿
...
1. **大拇指（landmark 4）与食指（landmark 8）捏合**  
   ...
   - 简化国家匹配，命中则在地图上标记地点，并在顶部**显示国家名**（或地区名）
...
#### 🖥️ UI 与体验
- 摄像头预览框位于右下角，用于确认手势是否被捕捉
- 顶部信息栏显示：“**当前选中：中国**” 或 “**请挥动手掌调整视角**” 等状态提示
- 若未检测到手部，显示“请将手放入摄像头视野”
- 所有动画（缩放、旋转）使用 `requestAnimationFrame` + 线性插值实现流畅过渡
- 自动请求摄像头权限，失败时显示错误
- 自定义地球旋转灵敏度，灵敏度范围0~2
- 手势提示信息在地球模型的下方
- 摄像头预览开关和旋转灵敏度在左下方
- 所有提示信息框的边界都用圆角展示，力求美观
